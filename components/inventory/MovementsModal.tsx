...
const MovementsModal: React.FC<MovementsModalProps> = ({ isOpen, onClose, item }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setLoading(true);
            api.getInventoryMovements(item.id)
                .then(setMovements)
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4">
            <div className="bg-surface rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col border border-neutral-border">
                <div className="p-4 border-b border-neutral-border flex justify-between items-center">
                    <h3 className="text-lg font-bold text-on-surface">Historial de Movimientos: <span className="text-primary">{item.name}</span></h3>
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-on-surface-variant/10">&times;</button>
                </div>
                <div className="p-4 overflow-y-auto">
                    ...
                    {!loading && movements.length === 0 && <p className="text-center text-on-surface-variant py-8">No hay movimientos registrados para este ítem.</p>}
                </div>
