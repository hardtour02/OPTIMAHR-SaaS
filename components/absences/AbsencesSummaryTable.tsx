                ...
                <table className="w-full text-sm text-left text-on-surface-variant">
                    ...
                    <tbody>
                        {requestsToDisplay.length > 0 ? requestsToDisplay.map((req) => (
                            <tr key={req.id} className="bg-surface border-b border-neutral-border last:border-b-0 hover:bg-primary-light-hover cursor-pointer" onClick={() => navigate(`/employee/${req.employeeId}`)}>
