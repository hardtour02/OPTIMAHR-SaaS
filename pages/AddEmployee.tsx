...
import { NewEmployee, Document, FormFieldOption, Company, CustomFieldDef, ContractType } from '../types';
...
const initialFormData: NewEmployee = {
    photoUrl: '', firstName: '', lastName: '', idNumber: '', phone: '', email: '', company: '',
    status: 'active', gender: 'Masculino', civilStatus: 'Soltero/a', birthDate: '', bloodType: 'A+',
    nationality: '', licenciaDeConducir: 'No', hierarchy: '', title: '', 
    minSalary: 0, currentSalary: 0, maxSalary: 0, 
    zonaDeTrabajo: 'oficina',
    contractStartDate: '', contractEndDate: '', contractTypeId: '', country: '', state: '', parish: '',
    emergencyContact: { name: '', phone: '' }, documents: [],
};
...
const AddEmployee: React.FC = () => {
    ...
    const [formData, setFormData] = useState<NewEmployee | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [optionsData, companiesData, customFieldsData, contractsData] = await Promise.all([
                ...
                setContractTypes(contractsData.filter(c => c.isActive));
                
                const initialWithCustom = customFieldsData.reduce((acc, field) => {
                    acc[field.label] = '';
                    return acc;
                }, { ...initialFormData });
                setFormData(initialWithCustom);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Error al crear el empleado. Intente de nuevo.');
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <h1 className="text-3xl font-bold text-on-surface">Agregar Nuevo Empleado</h1>
